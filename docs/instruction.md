...
- Create and EC2 instance > docker container > mcr.microsoft.com/mssql/server:2025-latest
- Spin up the Legacy MSSQL Server
```
docker run -e "ACCEPT_EULA=Y" -e "MSSQL_SupplyChain_PASSWORD=LogisticAdv123" -p 1433:1433 --name legacy-mssql -d mcr.microsoft.com/mssql/server:2025-latest

# or multi-line in windows 
docker run -e "ACCEPT_EULA=Y" -e "MSSQL_SupplyChain_PASSWORD=LogisticAdv123!" ^
...
docker run -v mssql_data:/var/opt/mssql \
  -e "ACCEPT_EULA=Y" \
  -e "MSSQL_SupplyChain_PASSWORD=LogisticAdv123!" \
  -p 1433:1433 \
  --name legacy-mssql \
  -d mcr.microsoft.com/mssql/server:2025-latest
